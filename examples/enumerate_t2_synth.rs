//! T2 SYNTHETIC variant — runs the pair-trading enumeration on two
//! independent 100k-bar GBM streams (seeds 11 + 22) instead of BTC/DOGE.
//! Output → /home/daru/leak_test/strategies/T2_synth/.
...
fn build_btc_doge_panel() -> PanelData {
    use std::io::BufRead;
    let mut tmpdir = std::env::temp_dir(); tmpdir.push("t2_synth_panel");
    fs::create_dir_all(&tmpdir).unwrap();
    let src_btc  = "/home/daru/leak_test/synth_data/synth_A_eng.csv";
    let src_doge = "/home/daru/leak_test/synth_data/synth_B_eng.csv";
    ...
    let btc_path  = tmpdir.join("SYNTH_A.csv");
    let doge_path = tmpdir.join("SYNTH_B.csv");
    ...
    drop(bw); drop(dw);
    load_panel(&[
        ("SYNTH_A".to_string(), btc_path.clone()),
        ("SYNTH_B".to_string(), doge_path.clone()),
    ]).expect("load_panel")
}
...
fn main() {
    ...
    let n = panel.times.len();
    eprintln!("[shard {}/{}] loaded SYNTH-A/B 30m panel: {} aligned bars", shard, total_shards, n);
    ...
    let out_root = PathBuf::from("/home/daru/leak_test/strategies/T2_synth");
