...
// ============================================================================
fn compute_ema(close: &[f64], span: usize) -> Vec<f64> {
...
// 3. RAW SIGNALS
// ============================================================================
fn create_raw_signals(close: &[f64], lb: usize) -> Vec<i8> {
    let n = close.len();
    let ema20 = compute_ema(close, FAST_EMA_SPAN);
    let ema_lb = compute_ema(close, lb);
    let mut raw = vec![0i8; n];
    for i in 1..n {
        if ema20[i - 1] > ema_lb[i - 1] { raw[i] = 1; }
        else if ema20[i - 1] < ema_lb[i - 1] { raw[i] = -1; }
    }
    raw
}
...
fn optimiser(bars: &[Bar], cfg: &mut Config) -> (Option<usize>, Metrics) {
    ...
    let mut evaluate = |lb: usize, cfg: &mut Config, cache: &mut HashMap<usize, Option<(f64, usize, Metrics)>>| -> Option<(f64, usize, Metrics)> {
        ...
        let raw = create_raw_signals(&close, lb);
    ...
    if coarse_results.is_empty() {
        ...
        let raw = create_raw_signals(&close, DEFAULT_LB);
        let sig = parse_signals(&raw);
        let (_, m, _, _) = run_backtest(bars, &sig, cfg);
...
fn classic_single_run(all_bars: &[Bar], cfg: &mut Config) -> ClassicResult {
    ...
    // RAW baseline
    let raw_is = create_raw_signals(&is_close, DEFAULT_LB);
    ...
    let raw_oos = create_raw_signals(&oos_close, DEFAULT_LB);
    ...
    // Optimise
    let (best_lb, met_is_opt) = optimiser(is_bars, cfg);
    ...
    if let Some(lb) = best_lb {
        ...
        export_trades(&tr_is_opt, is_bars, "EMA-crossover", &format!("LB{}", lb), "IS-opt", export_path, true);
        prettyprint("OOS-opt", &met_oos_opt, Some(lb));
        export_trades(&tr_oos_opt, oos_bars, "EMA-crossover", &format!("LB{}", lb), "OOS-opt", export_path, false);
...
fn run_wfo_window(is_bars: &[Bar], oos_bars: &[Bar], lb: usize, window_tag: &str,
    cfg: &Config, rb_scenarios: &[(String, RobustnessOpts)], export_is: bool,
) -> (Vec<f64>, Vec<f64>) {
    let export_path = "trade_list.csv";
    let is_close: Vec<f64> = is_bars.iter().map(|b| b.close).collect();
    let oos_close: Vec<f64> = oos_bars.iter().map(|b| b.close).collect();

    let raw_is = create_raw_signals(&is_close, lb);
    let sig_is = parse_signals(&raw_is);
    let (tr_is, met_is, eq_is, _) = run_backtest(is_bars, &sig_is, cfg);

    let raw_oos = create_raw_signals(&oos_close, lb);
    let sig_oos = parse_signals(&raw_oos);
    ...
    if export_is {
        export_trades(&tr_is, is_bars, "EMA-crossover-WFO", window_tag, "IS", export_path, header_needed);
    }
    let header_needed2 = !Path::new(export_path).exists();
    export_trades(&tr_oos, oos_bars, "EMA-crossover-WFO", window_tag, "OOS", export_path, if export_is { false } else { header_needed2 });
    ...
    for (label, opts) in rb_scenarios {
        ...
        let raw_is_rb = create_raw_signals(&is_close, lb_rb);
        ...
        let raw_oos_rb = create_raw_signals(&oos_close, lb_rb);
...
fn walk_forward(all_bars: &[Bar], eq_is_baseline: &[f64], cfg: &mut Config) {
    ...
    while cur_start < ni {
        let cur_end: i64 = if WFO_TRIGGER_MODE == "candles" {
            ...
            let (lb_roll, _) = optimiser(is_bars_roll, cfg);
            if lb_roll.is_none() { break; }
            let lb = lb_roll.unwrap();
            let oos_remaining = &all_bars[cs_idx..n];
            let oos_close: Vec<f64> = oos_remaining.iter().map(|b| b.close).collect();
            let raw_tmp = create_raw_signals(&oos_close, lb);
        ...
        let (rets_oos, eq_is_window) = run_wfo_window(
            ...
            cfg, &rb_scenarios_parsed, window_no == 1);
...
// ROBUSTNESS TESTS
// ============================================================================
fn run_robustness_tests(all_bars: &[Bar], best_lb: Option<usize>, best_rrr: Option<usize>, cfg: &Config) {
    let scenarios = robustness_scenarios();
    for (name, flags) in scenarios.iter().take(MAX_ROBUSTNESS_SCENARIOS) {
        ...
        let raw_is = create_raw_signals(&is_close, lb_use);
        ...
        let raw_oos = create_raw_signals(&oos_close, lb_use);
...
fn main() {
    ...
    let csv_path = std::env::args().nth(1).unwrap_or_else(|| CSV_FILE.to_string());
    if !Path::new(&csv_path).exists() {
        panic!("CSV file not found: {}\n\nPut an OHLC CSV at that path, or pass one as the first CLI arg.\nYou can use any Binance-style CSV: time (unix seconds),open,high,low,close.", csv_path);
    }
    ...
    let base = classic_single_run(&bars, &mut cfg);
    ...
    run_robustness_tests(&bars, base.best_lb, base.best_rrr, &cfg);

    if USE_WFO {
        println!("\n Running Walk-Forward Windows ");
        // Python WFO uses default TP (classic_single_run restores globals)
        walk_forward(&bars, &base.eq_is_raw, &mut cfg);
    }
